...
df['zip_code'] = pd.to_numeric(df['zip_code'], errors='coerce').fillna(0).astype(int).astype(str)
...
df_merged = df.merge(df_acs, left_on='zip_code', right_on='ZIP Code', how='left')
...
missing_census_zips = set(df_acs['ZIP Code'].unique()) - set(df['zip_code'].unique())
